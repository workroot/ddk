<?php
namespace app\calculator\lawyerpays;

use app\calculator\lawyerpays\lib\WxPayApi;
use app\calculator\lawyerpays\lib\WxPayNotify;
use Con\Con;
use think\Db;
...
class Notify extends WxPayNotify
{
    ...
    {
            if(!empty($order)){
                $result = db('LawyerOrder')->where('id', $order['id'])->update(['status'=>1, 'transaction_id'=>$data['transaction_id']]);
                // $product = Db::name("LawyerOrder")->where(array("out_trade_no"=>$out_trade_no))->where('status','=',1)->find();
                Db::name('clawyer')->where('id','=',$order['cid'])->setInc('awesome');
            }
          	return $result;
    }
    ...
    public function getOrder($data)
    {
        // 可根据商户订单号进行查询
        // 例如:
        $order = db('LawyerOrder')->where('out_trade_no', $data['out_trade_no'])->find();
        return $order;
    }
